...
def svm(cnj1, cnj2):
    ...
    if(clf_poly.score(features_cnj2, target_cnj2) > clf_rbf.score(features_cnj2, target_cnj2)):
        return clf_poly
    else:
        return clf_rbf
...
def findBestC(train_features, train_target, validation_features, validation_target):
    ...
    while i <= 20:
        clf_poly = make_pipeline(StandardScaler(), SVC(gamma='scale', kernel='poly', C=i, probability=True))
        clf_poly.fit(train_features, train_target)
        
        clf_rbf  = make_pipeline(StandardScaler(), SVC(gamma='scale', kernel='rbf', C=i, probability=True))
